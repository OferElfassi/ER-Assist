...
class ConversationListScreen extends Component {
  ...
  createItems(batch) {
    ...
    const map = _.map(data, (item, index) => {
      ...
      const avatarBadgeProps = {
        backgroundColor: Number(index) < 3 ? Colors.green30 : undefined,
      };
      ...
      const rightButtons = [
        {
          ...
          onPress: () =>
            Alert.alert(`More button pressed for item #${item.name}`),
        },
        {
          text: 'Archive',
          ...
          onPress: () =>
            Alert.alert(`Archive button pressed for item #${item.name}`),
        },
      ];
      const leftButton = {
        text: 'Read',
        ...
        onPress: () =>
          Alert.alert(`Read button pressed for item #${item.name}`),
      };
  ...
  onSwipeableWillOpen = itemProps => {
    this.closeLast(itemProps.index);
  };
